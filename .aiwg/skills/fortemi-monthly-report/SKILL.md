...
- **Post file:** `docs/content/posts/{{YYYY-MM}}-fortemi.md` (Pagenary `posts` collection, `route: /blog`, `layout: blog` — see `docs/config.json` → `collections`). Pagenary ≥ 2026.7.3 auto-materializes collection posts for our manifested tenant, so just dropping the file here is enough — no manifest edit needed.
