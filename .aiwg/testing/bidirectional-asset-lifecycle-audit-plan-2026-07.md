...
| Fortemi AL-PERF01 approved-budget receipt | versioned policy; immutable execution pending | `.aiwg/testing/asset-lifecycle-performance-policy-v1.json` approves exact percentile latency/throughput, RSS, disk, zero-byte RPO, RTO, 100 MiB, 28-sidecar, 64-entry, and bounded-I/O limits for the recorded Linux x86_64 filesystem scope. CI uses committed constants and the bundle verifier rejects any receipt limit that differs from policy; mutable repository variables can no longer create or weaken an approved receipt. Local approved runs passed at 1 MiB (`26/5/112/115` ms upload/download/export/import, 120 ms RTO, 25,333,760 byte RSS delta, 2,097,222 disk bytes) and 100 MiB (`263/143/2280/622` ms, 757 ms RTO, 458,088,448 byte RSS delta, 209,715,270 disk bytes). |
| Fortemi AL-PERF01 CI artifact wiring | prior immutable baseline passed; policy run pending | `.gitea/workflows/ci-builder.yaml` captures the default receipt, five clean-checkout samples, derived policy-evaluated `observed-percentiles.json`, clean-checkout receipt, approved 100 MiB receipt, exact max-count receipt, approved TUS-memory receipt, approved 1 MiB receipt, and manifest. The verifier requires phase snapshots, exact corpus/count roles, bounded I/O, exact policy values, five-sample agreement, policy percentile pass, immutable baseline comparison, clean checkout, conservative cross-client claims, and exact file digests. Run 6183 job 104296 remains the immutable historical baseline. A successful current-head policy artifact is still required before #1094 closure. |
...
- AL-PERF01 has a finalized immutable CI artifact for the default 1 MiB corpus, throughput fields,
  ...
  missing manifest, stale manifest, and write-manifest mode. The current-head immutable policy
...
- Immutable current-head publication of the approved AL-PERF01 policy bundle.
...
| AL-PERF01 | Fortemi + HotM | Execute approved size/count corpus with RSS, disk, latency, and throughput capture. | Partial pending current-head artifact: performance policy revision 1 approves exact p50/p95/p99 latency and throughput, RSS, disk, zero-byte RPO, timed RTO, 100 MiB, 28-sidecar/64-entry, and bounded filesystem TUS/full-v1 limits for Linux x86_64. `al_perf01_configurable_corpus_records_receipt_and_limit_plus_one_gate` records every lifecycle phase's high-water RSS and storage/TUS disk, timings, throughput, cleanup, limit-plus-one pre-mutation rejection, exact recovery, and policy evaluations. The process-isolated 1 MiB/100 MiB TUS path passes its approved RSS-growth limits. The statistical receipt exact-recomputes five clean-checkout samples, evaluates every percentile, and compares with immutable run 6183 without allowing the trend to weaken policy. Local approved 1 MiB and 100 MiB runs passed. Run 6183/job 104296 remains the immutable pre-policy baseline; a current-head finalized policy bundle is required. Whole-lifecycle bounded memory, scanner/non-filesystem and non-Linux evidence, immutable HotM browser publication, and suite-wide portability remain false. |
