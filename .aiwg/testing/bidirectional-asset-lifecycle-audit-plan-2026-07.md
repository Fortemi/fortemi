...
| Fortemi AL-SYS04/05 scoped CI receipt | local receipt passed; immutable run pending | `scripts/ci/verify-asset-lifecycle-system-receipt.sh` reruns the exact live route, two-sidecar process-abort, journal-recovery, and filesystem-refcount commands before writing `fortemi.asset-lifecycle.system-receipt.v1`. The verifier binds the clean Git commit, relative source paths and SHA-256 values, Linux/architecture/filesystem context, headless execution, isolated PostgreSQL schema and filesystem storage modes, explicit `disabled-isolated-ci` authentication mode, the exact HotM live Tauri consumer receipt commits and SHA-256, and the 30-day Gitea artifact identity. It rejects source or test inventory drift, dirty clean-checkout claims, sensitive connection/credential/absolute-path patterns, and any true mid-syscall, kernel/fsync, power-loss, commit-ambiguity, authenticated, non-Unix, platform-matrix, or suite-portability claim. Six focused verifier tests and the complete local receipt-producing command passed. `.gitea/workflows/ci-builder.yaml` now uploads only the sanitized JSON as `al-sys04-05-asset-lifecycle-receipt`; successful immutable execution is still required. |
...
- Successful immutable CI artifact publication for the dedicated Fortemi
  `al-sys04-05-asset-lifecycle-receipt`.
