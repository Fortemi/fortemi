...
| Fortemi AL-SYS04/05 scoped CI receipt | passed: Gitea run `6193`, job `104450`, commit `64f7c117345727bca60e40b7c703576b88db4bea` | `scripts/ci/verify-asset-lifecycle-system-receipt.sh` reran the exact live route, two-sidecar process-abort, journal-recovery, and filesystem-refcount commands before writing and verifying `fortemi.asset-lifecycle.system-receipt.v1` on a clean checkout. The verifier bound the exact commit, relative source paths and SHA-256 values, Linux/architecture/filesystem context, headless execution, isolated PostgreSQL schema and filesystem storage modes, explicit `disabled-isolated-ci` authentication mode, the exact HotM live Tauri consumer receipt commits and SHA-256, and the 30-day Gitea artifact identity. It rejects source or test inventory drift, dirty clean-checkout claims, sensitive connection/credential/absolute-path patterns, and any true mid-syscall, kernel/fsync, power-loss, commit-ambiguity, authenticated, non-Unix, platform-matrix, or suite-portability claim. Lint job `104443` passed all six focused verifier tests. Build job `104450` verified the receipt at `2026-07-28T04:29:09Z`, finalized `al-sys04-05-asset-lifecycle-receipt` at `2026-07-28T04:29:11Z`, and completed successfully at `2026-07-28T04:30:19Z`. |
| Fortemi AL-PERF01 1 MiB receipt scaffold | 1/1 | Configurable corpus defaults to 1 MiB and records upload/download/export/import timing and throughput, archive size and digest, zero-byte-loss signed full-v1 RPO, import+verify-download RTO, package/platform/filesystem context, and limit-plus-one TUS rejection before mutation. It now also records cumulative high-water RSS plus storage/TUS disk snapshots before work and after upload, source download, signed export, clean import, and recovery download; every completed phase requires zero TUS staging residue. |
...
| Fortemi AL-PERF01 process-isolated TUS memory guard | 2/2 Linux child-process corpora | `al_perf01_process_isolated_streamed_tus_memory_guard` runs separate 1 MiB and 100 MiB child processes, generates 64 KiB request frames on demand, and measures `/proc/self/status` immediately around filesystem TUS PATCH/finalization. Both children prove exact database hash/size/refcount, bounded-file hash/size, and zero staging residue. Performance policy revision 1 approves at most 64 MiB RSS high-water delta for the 100 MiB filesystem TUS path and at most 32 MiB growth over the 1 MiB control. The current local policy run recorded 2,838,528 and 4,210,688 bytes, or 1,372,160 bytes of growth. Whole lifecycle, scanner, non-filesystem, non-Linux, and suite-portability claims remain false. |
| Fortemi AL-PERF01 clean-checkout statistical receipt | configured five-sample policy gate + 23/23 verifier tests | CI runs five 1 MiB lifecycle samples with `FORTEMI_AL_PERF_EXPECT_CLEAN_CHECKOUT=1`. `observed-percentiles.json` exact-recomputes nearest-rank p50/p95/p99 for phase latency, four throughput fields, recovery RTO, RSS high-water delta, and storage-plus-TUS disk. Performance policy revision 1 defines explicit limits for every percentile, exact single-run and 100 MiB budgets, zero-byte RPO, RTO, corpus/count boundaries, and bounded TUS I/O. The receipt binds the policy and immutable run 6183 baseline by SHA-256, records per-percentile pass evaluations and baseline deltas, and rejects budget drift or regression. Historical comparison cannot relax policy. Whole-lifecycle bounded memory and suite portability remain false. |
...
| HotM live Tauri signed recovery | immutable run 1168 / job 104771: 1/1 passed | HotM commit `6e16f5373f75e8eb17bfb1290f73d5a456357c24` against Fortemi `51a0e6c4ed33874ce34a5ae799a41801b81f431d` authenticated through OAuth client credentials, uploaded deterministic production Tauri command-core bytes over filesystem TUS, reopened the source bytes, exported signed `2.0.0/full-v1`, deleted the source archive and proved it unavailable before recovery, imported into a distinct clean recovery archive with `verify_signature=require`, and matched source/recovery byte count, SHA-256, and BLAKE3. The combined receipt also binds the browser and desktop normalized contract. This is a headless production command-core receipt; it does not claim a launched GUI, interactive dialogs in the same run, API-process termination, non-Linux execution, or suite portability. |
...
| Fortemi AL-PERF01 approved-budget receipt | immutable current-policy execution passed | `.aiwg/testing/asset-lifecycle-performance-policy-v1.json` approves exact percentile latency/throughput, RSS, disk, zero-byte RPO, RTO, 100 MiB, 28-sidecar, 64-entry, and bounded-I/O limits for the recorded Linux x86_64 filesystem scope. CI uses committed constants and the bundle verifier rejects any receipt limit that differs from policy; mutable repository variables can no longer create or weaken an approved receipt. Exact policy commit `986fc9d04e92de748122948f9f04a760ad63ab9d` plus stabilization commit `51a0e6c4ed33874ce34a5ae799a41801b81f431d` passed Gitea run `6207`, job `104656`. |
| Fortemi AL-PERF01 CI artifact wiring | current policy bundle passed: run 6207 / job 104656 | `.gitea/workflows/ci-builder.yaml` captures the default receipt, five clean-checkout samples, derived policy-evaluated `observed-percentiles.json`, clean-checkout receipt, approved 100 MiB receipt, exact max-count receipt, approved TUS-memory receipt, approved 1 MiB receipt, and manifest. The verifier requires phase snapshots, exact corpus/count roles, bounded I/O, exact policy values, five-sample agreement, policy percentile pass, immutable baseline comparison, clean checkout, conservative cross-client claims, and exact file digests. Run `6207`, job `104656`, at `51a0e6c4ed33874ce34a5ae799a41801b81f431d` finalized the current-policy bundle; run `6183`, job `104296`, remains its immutable historical baseline. |
...
| HotM live browser metrics artifact | immutable run 1168 / job 104771: 4/4 passed | At HotM `6e16f5373f75e8eb17bfb1290f73d5a456357c24`, the enabled Playwright run passed 4/4 against isolated Fortemi `51a0e6c4ed33874ce34a5ae799a41801b81f431d` with OAuth bearer enforcement, PostgreSQL 18 plus pgvector/PostGIS, writable filesystem/TUS storage, deterministic signing/trust, and clean source/recovery archives. It used UI `setInputFiles`, saved a recovered browser download, interrupted the browser TUS stream at two increasing offsets, confirmed each offset with authenticated HEAD, completed exactly one digest-correct attachment, and proved server-origin re-upload and shard recovery agree on bytes, SHA-256, length, safe filename/media metadata, note ownership, attachment/blob relationships, and normalized browser/desktop contract. The combined verifier's 16 focused tests, 3 Playwright schema guards, redaction guards, receipt manifest, and full local harness passed. Launched desktop GUI, interactive dialogs, non-Linux execution, and suite portability remain false. |
...
| HotM combined receipt publication | run 1168 / job 104771 passed | `.gitea/workflows/live-asset-receipt.yml` ran `scripts/ci/run-live-asset-receipt.sh` on the lifecycle-capable builder against exact Fortemi `51a0e6c4ed33874ce34a5ae799a41801b81f431d`. The isolated authenticated job ran the enabled headless browser journey 4/4 and the ignored production Tauri command-core journey 1/1, verified the combined exact-commit receipt and redacted manifest, and finalized six files as `hotm-live-asset-ci-receipt` at `2026-07-28T07:36:09Z` (8,357 raw bytes; 3,684 uploaded bytes). The job completed successfully at `2026-07-28T07:36:10Z` and removed its container and volumes. |
...
in-flight commit acknowledgement ambiguity, full Tauri desktop GUI local-file journeys,
whole-lifecycle bounded memory, scanner/non-filesystem backends, non-Linux platforms, or suite
portability. Fortemi's approved scoped AL-PERF01 policy bundle is immutable in run `6207`, job
`104656`; run `6183`, job `104296`, remains the immutable historical baseline. HotM's combined
authenticated headless browser and Tauri command-core receipt passed immutable run `1168`, job
`104771`, at exact consumer and producer commits.
...
- HotM browser `setInputFiles`, TUS client state, browser download, browser re-upload, and signed
  ...
  download+hash verification. The current-head authenticated receipt now performs two increasing
  HEAD-confirmed resume checkpoints, proves exactly one final attachment, and compares re-upload
  and shard-recovered attachment/note/blob relationships. Both the Playwright writer and combined
  CI receipt job fail closed when completed metrics evidence is missing or under-asserted;
  immutable run `1168`, job `104771`, passed and finalized the combined artifact. The Tauri
  ...
  separate launched Linux receipt. A clean-worktree authenticated headless receipt now drives
  those production upload/download cores against exact live Fortemi through filesystem TUS, signed
  `2.0.0/full-v1`, trust-required clean-memory import, source-archive deletion plus a 404 oracle
  before recovery, and exact reopened source/recovery bytes. A launched GUI plus interactive native
  dialogs and signed recovery in the same run and non-Linux platforms remain open.
...
- AL-PERF01 has a finalized immutable CI artifact for the default 1 MiB corpus, throughput fields,
  ...
  clean-checkout p50/p95/p99 observations with exact source digests and a stable trend key.
  Performance policy revision 1 now approves exact latency, throughput, RSS, disk, zero-byte RPO,
  timed RTO, 100 MiB, 28-sidecar, and bounded-I/O limits for the recorded Linux x86_64 filesystem
  scope. Server-side filesystem TUS request/finalization and full-v1 sidecar I/O have explicit
  8 KiB prefix and 64 KiB stream/copy bounds with fail-closed receipt validation. A separate Linux
  child-process policy receipt shows 1,372,160 bytes of RSS high-water growth from the 1 MiB control
  to the 100 MiB TUS corpus. Whole asset-lifecycle process bounded memory remains false because the
  lifecycle measurement harness retains corpus/archive vectors. The 100 MiB deterministic
  ...
  `al-perf01-asset-lifecycle-receipts` artifact and is the immutable historical baseline.
  Current CI always emits approved 1 MiB and 100 MiB receipts from committed policy values; the
  verifier rejects not-configured branches, threshold drift, measured-value drift, or failed
  budgets. Each route receipt records cumulative RSS and disk snapshots across every lifecycle
  phase and requires zero completed-phase TUS residue. The CI artifact bundle verifier has unit
  coverage for not-configured rejection, missing clean-checkout receipt, dirty clean-checkout
  receipt, approved-budget drift/missing-max/failed-RTO receipts, incomplete phase evidence, missing
  platform/filesystem context, false max-corpus claim, missing/understated TUS memory receipt,
  missing/dirty statistical samples, policy percentile regression, tampered p50/p95/p99 output,
  missing manifest, stale manifest, and write-manifest mode. Run `6207`, job `104656`, finalized
  and verified the current-policy artifact at `51a0e6c4ed33874ce34a5ae799a41801b81f431d`.
  HotM client-boundary metrics and production Tauri local-file command cores are proven together
  by authenticated exact-commit immutable run `1168`, job `104771`, against live Fortemi signed
  `2.0.0/full-v1` recovery with exact reopened bytes and attachment/note/blob relationship
  convergence. Launched GUI and suite portability remain false.
...
- Declared non-Linux/platform/filesystem matrix and scanner/non-filesystem backend memory evidence.

## Required System Tests

| ID | Owner | Required flow | Green oracle |
|---|---|---|---|
| AL-SYS01 | HotM + Fortemi | Launch isolated Fortemi and HotM desktop uploader; upload deterministic file over real TUS; export signed `2.0.0/full-v1`; stop source; import clean; download locally. | Partial with immutable headless run `1168` / job `104771`: HotM `6e16f5373f75e8eb17bfb1290f73d5a456357c24` drives authenticated production Tauri local-file upload/download command cores against Fortemi `51a0e6c4ed33874ce34a5ae799a41801b81f431d`, filesystem TUS, signed `2.0.0/full-v1`, source-archive deletion plus GET 404 before `verify_signature=require` import into a distinct clean recovery archive, and exact source/sidecar/storage/download byte count, SHA-256, BLAKE3, and cleanup. This proves logical source archive retirement, not API-process termination. Separate launched Linux evidence proves the dialog plugin. A launched GUI plus interactive picker/save and signed recovery in one run, non-Linux platforms, and suite portability remain required. |
| AL-SYS02 | HotM + Fortemi | Playwright `setInputFiles` against HotM connected to live Fortemi; export/import clean; save download to a file. | Covered in immutable headless run `1168` / job `104771`: the current-head OAuth-authenticated Playwright receipt uses UI `setInputFiles`, browser download saved to a test-controlled file, signed `2.0.0/full-v1` export, clean recovery import, and recovered-byte verification. It interrupts browser TUS at two increasing offsets, authenticates each HEAD confirmation, resumes, and commits exactly one digest-correct attachment. Browser and Tauri command-core receipts converge on the same normalized server contract. A launched Tauri desktop/native file-save receipt and non-Linux execution remain outside this headless receipt. |
| AL-SYS03 | HotM + Fortemi | Seed server attachment; download to clean filesystem; upload to clean server and independently recover source via signed `full-v1`. | Covered in immutable headless run `1168` / job `104771`: the current-head authenticated browser receipt seeds a server-origin attachment, downloads and saves exact bytes, re-uploads to a clean note, independently recovers through signed `full-v1`, and proves both destinations agree on attachment ID, note ownership, blob relationship, canonical/original safe filenames, byte count, SHA-256, content type, content disposition, and exact bytes. Tauri command core independently saves and reopens authenticated server bytes. Launched native desktop GUI operation, non-Linux platforms, and suite portability remain outside this bounded receipt. |
...
| AL-SYS05 | Fortemi | Concurrent identical-byte upload, archive import, and selected-reference deletion. | Covered by immutable Gitea run `6193`, job `104450`, at `64f7c117345727bca60e40b7c703576b88db4bea`; the scoped receipt reran and bound the concurrency oracle, isolated PostgreSQL schema, filesystem storage, exact sources, and conservative claim exclusions. |
| AL-PERF01 | Fortemi + HotM | Execute approved size/count corpus with RSS, disk, latency, and throughput capture. | Bounded Fortemi policy cell passed in immutable run `6207`, job `104656`, at `51a0e6c4ed33874ce34a5ae799a41801b81f431d`: performance policy revision 1 covers exact p50/p95/p99 latency and throughput, RSS, disk, zero-byte RPO, timed RTO, 100 MiB, 28-sidecar/64-entry, and bounded filesystem TUS/full-v1 limits for Linux x86_64. The process-isolated TUS path passes its approved RSS-growth limits and the statistical receipt exact-recomputes five clean-checkout samples against immutable baseline run `6183`, job `104296`, without allowing history to weaken policy. HotM immutable run `1168`, job `104771`, adds authenticated browser/Tauri client-boundary timings and exact recovery but does not approve a whole-lifecycle memory budget. Whole-lifecycle bounded memory, scanner/non-filesystem and non-Linux evidence, and suite portability remain false. |
