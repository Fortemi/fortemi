...
| CERT-004 | Certificate validation MUST verify that the issuing MPC wallet is reachable and has not revoked its own group key. If the issuing wallet is unreachable, the certificate SHOULD be accepted for read-only operations only, with a `wallet_unreachable` warning flag. Write operations MUST be rejected when wallet liveness cannot be confirmed. | SHOULD | Integration test: validate cert with wallet reachable; verify full access. Simulate wallet unreachable; verify read operations succeed with warning. Verify write operations fail with `WalletUnreachable` error. Verify warning is surfaced in API response metadata. |
