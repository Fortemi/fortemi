...
- [~] **#910** AuditEvent / TracingSink / bounded AuditBuffer — audit baseline. (2026-06-25: first core slice added `AuditEvent`, `AuditSink`, `TracingSink`, bounded `AuditBuffer`, event failure policies, pre-buffer redaction/sanitization, a low-risk process startup producer, and a mapping note for localized audit tables. 2026-06-26: PKE keyset lifecycle slice emits metadata-only `fortemi.audit` events for create/import/activate/delete/export without passphrases, key material, ciphertext, or exported payloads.)
