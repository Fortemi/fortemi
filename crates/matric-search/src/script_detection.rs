...
pub enum DetectedScript {
    ...
    /// Emoji characters
    Emoji,
    /// Math and technical symbols (∑, ∏, ∫, √, etc.) - requires trigram search
    Symbol,
...
pub fn detect_script(query: &str) -> ScriptDetection {
    ...
    for ch in query.chars() {
        ...
        if ch.is_whitespace() || (ch.is_ascii_punctuation() && !is_emoji(ch) && !is_symbol(ch)) {
            continue;
        }
        ...
        // Check for math/technical symbols (requires trigram search)
        if is_symbol(ch) {
            *script_counts.entry(DetectedScript::Symbol).or_insert(0) += 1;
            total_count += 1;
            continue;
        }
...
/// Checks if the query contains math or technical symbols.
///
/// These symbols require trigram search since FTS doesn't tokenize them.
///
/// # Examples
///
/// ```
/// use matric_search::script_detection::has_symbols;
///
/// assert!(has_symbols("∑ x²"));
/// assert!(has_symbols("€100"));
/// assert!(!has_symbols("Hello world"));
/// ```
pub fn has_symbols(query: &str) -> bool {
    query.chars().any(is_symbol)
}
...
/// Determines if a character is a math or technical symbol.
///
/// Math symbols require trigram search because PostgreSQL FTS doesn't tokenize them.
/// Includes: ∑ ∏ ∫ √ ∞ ≠ ≤ ≥ ± × ÷ and currency symbols like € £ ¥ ₹ ₿
fn is_symbol(ch: char) -> bool {
    matches!(ch as u32,
        ...
        0x00A2..=0x00A5 | // ¢ £ ¤ ¥ (Latin-1 Supplement currency)
...
mod tests {
    ...
    #[test]
    fn test_detect_math_symbols() {
        let result = detect_script("∑ ∏ ∫");
        assert_eq!(result.primary, DetectedScript::Symbol);
        assert!(result.confidence > 0.99);
    }
    ...
    #[test]
    fn test_has_symbols_math() {
        assert!(has_symbols("∑ x²"));
        assert!(has_symbols("x ≠ y"));
        assert!(has_symbols("∫ f(x) dx"));
    }

    #[test]
    fn test_has_symbols_currency() {
        assert!(has_symbols("€100"));
        assert!(has_symbols("Price: £50"));
        assert!(has_symbols("¥1000"));
    }

    #[test]
    fn test_has_symbols_pure_text() {
        assert!(!has_symbols("Hello world"));
        assert!(!has_symbols("Just text"));
    }

    #[test]
    fn test_has_symbols_empty() {
        assert!(!has_symbols(""));
    }

    #[test]
    fn test_mixed_latin_and_symbols() {
        // Text with both Latin and symbols - should detect as Mixed if symbols are significant
        let result = detect_script("Sum: ∑ ∏ ∫ √ ∞");
        assert!(result.scripts_found.contains(&DetectedScript::Symbol));
    }
}
