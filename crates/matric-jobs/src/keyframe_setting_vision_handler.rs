//! KeyframeSettingVisionHandler — identifies place, setting, and objects in a video keyframe.
//!
//! Specialized vision pass that identifies the environment, location, landmarks,
//! and notable objects in each keyframe. Stores results in
//! extracted_metadata.setting_analysis rather than ai_description.
//!
...
fn keyframe_setting_text_len(text: &str) -> usize {
    text.chars().count()
}

fn keyframe_setting_error_reason_code(error: &str) -> &'static str {
...
pub struct KeyframeSettingVisionHandler {
    db: Database,
    vision: Option<Arc<dyn VisionBackend>>,
}

impl KeyframeSettingVisionHandler {
...
#[async_trait]
impl JobHandler for KeyframeSettingVisionHandler {
    ...
    async fn execute(&self, ctx: JobContext) -> JobResult {
        let payload = match ctx.payload() {
            Some(p) => p.clone(),
            None => return JobResult::Failed("Missing keyframe setting vision job payload".into()),
        };
        ...
        let vision = match self.vision.as_ref() {
            ...
            None => {
                warn!(
                    ...
                    "KeyframeSettingVision job deferred — vision backend unavailable"
                );
            ...
            let mut tx = match schema_ctx.begin_tx().await {
                ...
                Err(e) => {
                    ...
                    return JobResult::Failed(format!(
                        "Schema tx failed ({})",
                        keyframe_setting_error_reason_code(&error_text)
                    ));
                }
            };
        ...
        // Step 2: Call vision LLM with setting-specific prompt
        ctx.report_progress(
            30,
            Some(&format!(
                "Analyzing setting in frame {}/{} ({:.0}s)",
                frame_index + 1,
                total_frames,
                timestamp_secs
            )),
        ...
        let prompt = "Identify the setting, location, and notable objects in this video frame. \
            Describe: environment (indoor/outdoor, type of space), identifiable \
            landmarks or signage, key objects and props, overall atmosphere. \
            Focus on what the space and objects reveal about the scene's context.";
            ...
            .await
        {
            ...
            Err(e) => {
                ...
                warn!(
                    ...
                    keyframe_attachment_id_present = true,
                    model_len = keyframe_setting_text_len(vision.model_name()),
                    error_len = keyframe_setting_text_len(&error_text),
                    error_reason = keyframe_setting_error_reason_code(&error_text),
                    "Vision LLM failed for setting analysis — will retry"
                );
                return JobResult::Retry(format!(
                    "Vision LLM failed for setting analysis frame {} ({})",
                    frame_index,
                    keyframe_setting_error_reason_code(&error_text)
                ));
            }
        ...
        // Step 3: Store setting analysis in extracted_metadata
        ctx.report_progress(80, Some("Storing setting analysis"));
        {
            let mut tx = match schema_ctx.begin_tx().await {
                Ok(t) => t,
                Err(e) => {
            ...
            if let Err(e) = file_storage
                .merge_extracted_metadata_tx(
                    ...
                    &json!({
                        "setting_analysis": analysis,
                        "setting_analysis_complete": true,
                    }),
                )
                .await
            {
                let error_text = e.to_string();
                return JobResult::Failed(format!(
                    "Failed to store setting analysis ({})",
                    keyframe_setting_error_reason_code(&error_text)
                ));
            }
        ...
        info!(
            ...
            parent_attachment_id_present = true,
            keyframe_attachment_id_present = true,
            analysis_len = keyframe_setting_text_len(&analysis),
            "Keyframe setting analysis complete"
        );
    ...
    async fn check_fan_in(
        ...
        let vision_ready = if expected_passes >= 3 {
            ...
            debug!(
                ...
                total = total_frames,
                transcript_complete,
                "Setting fan-in: {}/{} fully analyzed + transcript={}",
                fully_analyzed,
                total_frames,
                transcript_complete
            );
                ...
                .await
            {
                Ok(Some(job_id)) => {
                    ...
                    info!(
                        total_frames,
                        job_id_present = true,
                        "All keyframes fully analyzed + transcript complete, KeyframeAssembly queued"
                    );
                }
                Ok(None) => {
...
mod tests {
    ...
    #[test]
    fn keyframe_setting_error_reason_code_uses_stable_classes() {
        assert_eq!(
            keyframe_setting_error_reason_code(
                "vision model failed for /home/operator/mm_key_secret"
            ),
            "model_backend_error"
        );
        assert_eq!(
            keyframe_setting_error_reason_code("postgres://user:secret@db/app sql failed"),
            "database_error"
        );
        assert_eq!(
            keyframe_setting_error_reason_code("Cannot connect to inference backend"),
            "connection_failed"
        );
        assert_eq!(
            keyframe_setting_error_reason_code("opaque backend text with token mm_key_secret"),
            "operation_failed"
        );
    }

    #[test]
    fn keyframe_setting_runtime_telemetry_helpers_redact_private_values() {
        let raw_error =
            "vision model postgres://user:pass@db.internal failed for /srv/private/mm_key_frame";
        let rendered = format!(
            "parent_attachment_id_present=true; keyframe_attachment_id_present=true; model_len={}; error_len={}; error_reason={}",
            keyframe_setting_text_len("private-model-mm_key"),
            keyframe_setting_text_len(raw_error),
            keyframe_setting_error_reason_code(raw_error)
        );
        ...
        assert!(!rendered.contains("mm_key_frame"));
    }
}
