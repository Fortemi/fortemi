...
async fn test_create_collection_with_nonexistent_parent() {
    ...
    assert!(result.is_err(), "creating with nonexistent parent should fail due to FK constraint");
...
async fn test_create_multiple_children_under_same_parent() {
    ...
    let children = collections.list(Some(parent_id)).await.expect("list children");
...
async fn test_list_at_each_hierarchy_level() {
    ...
    let root_id = collections.create(&unique_name("h-root"), None, None).await.unwrap();
    let l1_id = collections.create(&unique_name("h-l1"), None, Some(root_id)).await.unwrap();
    let l2a_id = collections.create(&unique_name("h-l2a"), None, Some(l1_id)).await.unwrap();
    let l2b_id = collections.create(&unique_name("h-l2b"), None, Some(l1_id)).await.unwrap();
    let l3_id = collections.create(&unique_name("h-l3"), None, Some(l2a_id)).await.unwrap();
...
async fn test_move_note_across_hierarchy_levels() {
    ...
    // Build hierarchy
    let root_id = collections.create(&unique_name("mv-root"), None, None).await.unwrap();
    let l1_id = collections.create(&unique_name("mv-l1"), None, Some(root_id)).await.unwrap();
    let l2_id = collections.create(&unique_name("mv-l2"), None, Some(l1_id)).await.unwrap();
    let l3_id = collections.create(&unique_name("mv-l3"), None, Some(l2_id)).await.unwrap();
    ...
    collections.move_note(note_id, Some(root_id)).await.expect("move to root");
    let root_notes = collections.get_notes(root_id, 100, 0).await.unwrap();
    assert!(root_notes.iter().any(|n| n.id == note_id), "note should be in root");

    // Move note: root → L2 (skip a level)
    collections.move_note(note_id, Some(l2_id)).await.expect("move to L2");
    let l2_notes = collections.get_notes(l2_id, 100, 0).await.unwrap();
    assert!(l2_notes.iter().any(|n| n.id == note_id), "note should be in L2");
    let root_notes = collections.get_notes(root_id, 100, 0).await.unwrap();
    assert!(!root_notes.iter().any(|n| n.id == note_id), "note should no longer be in root");

    // Move note: L2 → L3 (down one level)
    collections.move_note(note_id, Some(l3_id)).await.expect("move to L3");
    let l3_notes = collections.get_notes(l3_id, 100, 0).await.unwrap();
    assert!(l3_notes.iter().any(|n| n.id == note_id), "note should be in L3");

    // Move note: L3 → L1 (back up in hierarchy)
    collections.move_note(note_id, Some(l1_id)).await.expect("move to L1");
    let l1_notes = collections.get_notes(l1_id, 100, 0).await.unwrap();
    assert!(l1_notes.iter().any(|n| n.id == note_id), "note should be in L1");

    // Move note: L1 → uncategorized (None)
    collections.move_note(note_id, None).await.expect("move to uncategorized");
    let l1_notes = collections.get_notes(l1_id, 100, 0).await.unwrap();
    assert!(!l1_notes.iter().any(|n| n.id == note_id), "note should no longer be in L1");
...
async fn test_delete_leaf_collection() {
    ...
    let root_id = collections.create(&unique_name("del-root"), None, None).await.unwrap();
    let leaf_id = collections.create(&unique_name("del-leaf"), None, Some(root_id)).await.unwrap();
...
async fn test_delete_middle_collection_children_move_to_root() {
    ...
    // root → middle → child
    let root_id = collections.create(&unique_name("del2-root"), None, None).await.unwrap();
    let middle_id = collections.create(&unique_name("del2-mid"), None, Some(root_id)).await.unwrap();
    let child_id = collections.create(&unique_name("del2-child"), None, Some(middle_id)).await.unwrap();
    ...
    let child = collections.get(child_id).await.unwrap().expect("child should still exist");
    assert!(child.parent_id.is_none(), "child should have been moved to root (parent_id = NULL)");
...
async fn test_delete_collection_notes_become_uncategorized() {
    ...
    let coll_id = collections.create(&unique_name("del-notes"), None, None).await.unwrap();
    ...
    let note2_id = notes
        ...
        .await
        .unwrap();
    collections.move_note(note1_id, Some(coll_id)).await.unwrap();
    collections.move_note(note2_id, Some(coll_id)).await.unwrap();
    ...
    // Delete collection
    collections.delete(coll_id).await.expect("delete collection with notes");

    // Notes should still exist but have collection_id = NULL
    let note1 = notes.fetch(note1_id).await.expect("note1 should exist");
    assert!(note1.note.collection_id.is_none(), "note1 should be uncategorized");

    let note2 = notes.fetch(note2_id).await.expect("note2 should exist");
    assert!(note2.note.collection_id.is_none(), "note2 should be uncategorized");
...
async fn test_delete_root_of_deep_hierarchy() {
    ...
    // root → L1 → L2 → L3
    let root_id = collections.create(&unique_name("deep-root"), None, None).await.unwrap();
    let l1_id = collections.create(&unique_name("deep-l1"), None, Some(root_id)).await.unwrap();
    let l2_id = collections.create(&unique_name("deep-l2"), None, Some(l1_id)).await.unwrap();
    let l3_id = collections.create(&unique_name("deep-l3"), None, Some(l2_id)).await.unwrap();
    ...
    assert!(collections.get(root_id).await.unwrap().is_none(), "root should be deleted");

    // L1 should still exist with parent_id = NULL
    let l1 = collections.get(l1_id).await.unwrap().expect("L1 should exist");
    assert!(l1.parent_id.is_none(), "L1 should be promoted to root");

    // L2 should still point to L1
    let l2 = collections.get(l2_id).await.unwrap().expect("L2 should exist");
    assert_eq!(l2.parent_id, Some(l1_id), "L2 should still be under L1");

    // L3 should still point to L2
    let l3 = collections.get(l3_id).await.unwrap().expect("L3 should exist");
...
async fn test_delete_collection_with_both_notes_and_children() {
    ...
    let parent_id = collections.create(&unique_name("combo-parent"), None, None).await.unwrap();
    let child_id = collections.create(&unique_name("combo-child"), None, Some(parent_id)).await.unwrap();
    ...
    let note_id = notes
        ...
        .await
        .unwrap();
    collections.move_note(note_id, Some(parent_id)).await.unwrap();

    // Delete parent
    collections.delete(parent_id).await.expect("delete parent with notes and children");

    // Child should be promoted to root
    let child = collections.get(child_id).await.unwrap().expect("child should exist");
    ...
    assert!(note.note.collection_id.is_none(), "note should be uncategorized");
...
async fn test_collection_note_count() {
    ...
    let coll_id = collections.create(&unique_name("count-coll"), None, None).await.unwrap();
...
async fn test_move_collection_to_new_parent() {
    ...
    let root1_id = collections.create(&unique_name("mvcol-r1"), None, None).await.unwrap();
    let root2_id = collections.create(&unique_name("mvcol-r2"), None, None).await.unwrap();
    ...
    assert_eq!(child.parent_id, Some(root2_id), "child should now be under root2");
...
async fn test_move_collection_to_root() {
    ...
    let parent_id = collections.create(&unique_name("mvcol-p"), None, None).await.unwrap();
    let child_id = collections
    ...
    let child = collections.get(child_id).await.unwrap().unwrap();
    assert!(child.parent_id.is_none(), "child should now be a root collection");
...
async fn test_move_collection_with_subtree() {
    ...
    let root1_id = collections.create(&unique_name("mvtree-r1"), None, None).await.unwrap();
    let root2_id = collections.create(&unique_name("mvtree-r2"), None, None).await.unwrap();
    let parent_id = collections
...
async fn test_move_collection_to_self_fails() {
    ...
    let coll_id = collections.create(&unique_name("circ-self"), None, None).await.unwrap();
...
async fn test_move_collection_to_direct_child_fails() {
    ...
    // parent → child
    let parent_id = collections.create(&unique_name("circ-p"), None, None).await.unwrap();
    let child_id = collections
...
async fn test_move_collection_to_deep_descendant_fails() {
    ...
    // A → B → C → D
    let a_id = collections.create(&unique_name("circ-a"), None, None).await.unwrap();
    let b_id = collections.create(&unique_name("circ-b"), None, Some(a_id)).await.unwrap();
    let c_id = collections.create(&unique_name("circ-c2"), None, Some(b_id)).await.unwrap();
    let d_id = collections.create(&unique_name("circ-d"), None, Some(c_id)).await.unwrap();

    // Try to move A under D (would create: D → A → B → C → D → ...)
    let result = collections.move_collection(a_id, Some(d_id)).await;
    assert!(result.is_err(), "moving A under its deep descendant D should fail");
    let err_msg = result.unwrap_err().to_string();
    assert!(
        err_msg.contains("circular"),
        "error should mention circular reference: {}",
...
async fn test_move_collection_to_non_descendant_succeeds() {
    ...
    let a_id = collections.create(&unique_name("ok-a"), None, None).await.unwrap();
    let b_id = collections.create(&unique_name("ok-b"), None, Some(a_id)).await.unwrap();
    let _c_id = collections.create(&unique_name("ok-c"), None, Some(b_id)).await.unwrap();
    let d_id = collections.create(&unique_name("ok-d"), None, None).await.unwrap();
...
async fn test_update_preserves_parent_id() {
    ...
    let parent_id = collections.create(&unique_name("upd-parent"), None, None).await.unwrap();
    let child_id = collections
        .create(&unique_name("upd-child"), Some("Original desc"), Some(parent_id))
        .await
        .unwrap();
    ...
    let child = collections.get(child_id).await.unwrap().unwrap();
    assert_eq!(child.parent_id, Some(parent_id), "parent_id should be unchanged after update");
