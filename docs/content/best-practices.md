...
**Quick recommendation:** Use hierarchical path format with forward slashes (`/`). Keep depth under 4 levels. Apply PMEST faceted classification for systematic organization.

### Why Hierarchical Tags

Hierarchical tags with SKOS semantics provide:
- **Inheritance:** Search for `technology` finds notes tagged `technology/rust` and `technology/rust/async`
...
- **Clarity:** Path format shows context: `project/Fortémi/architecture`

### PMEST Faceted Classification

PMEST is a library science framework for systematic classification. Apply multiple facets to each note:

**Personality (Type)** - What kind of resource?
- `type/note` - Personal observation or idea
- `type/project` - Project-related content
- `type/reference` - External resource (book, article, video)
- `type/journal` - Time-series entry

**Matter (Source)** - Where did it come from?
- `source/book` - Book excerpt or summary
- `source/article` - Article, blog post, paper
- `source/video` - Video content
- `source/conversation` - Discussion, meeting notes

**Energy (Domain)** - What subject area?
- `domain/technology/programming/rust`
- `domain/business/strategy/marketing`
- `domain/science/physics/quantum`
...
- Solution: Choose one canonical term, create SKOS `broader`/`narrower` relations to merge
