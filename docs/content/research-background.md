...
This document provides the theoretical foundation for matric-memory's implementation, explaining the research basis for each major component with citations to peer-reviewed literature.
...
matric-memory implements hybrid retrieval using Reciprocal Rank Fusion (Cormack, Clarke, & Büttcher, 2009), combining rankings from both lexical and dense retrieval systems.
...
matric-memory uses sentence embeddings based on the Sentence-BERT architecture (Reimers & Gurevych, 2019). Traditional BERT requires feeding both sentences through the transformer for comparison, making it O(n²) for finding similar documents. Sentence-BERT produces fixed-size embeddings that can be compared with simple cosine similarity.
...
matric-memory uses nomic-embed-text as a balance between quality and local inference speed via Ollama.
...
matric-memory uses HNSW (Hierarchical Navigable Small World) graphs (Malkov & Yashunin, 2020) via pgvector for approximate nearest neighbor search.
...
matric-memory implements automatic knowledge graph construction (Hogan et al., 2021) by discovering semantic relationships between notes.
...
matric-memory implements Retrieval-Augmented Generation (Lewis et al., 2020) for content enhancement:
...
matric-memory implements W3C SKOS (Simple Knowledge Organization System) for controlled vocabulary management (Miles & Bechhofer, 2009).

**Core Concepts:**

| SKOS Term | matric-memory | Purpose |
|-----------|---------------|---------|
