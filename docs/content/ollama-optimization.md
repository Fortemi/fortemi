...
This guide covers performance optimizations for Ollama inference backend used in Matric Memory.
...
Based on testing with Matric Memory workloads:
...
Matric Memory's inference backend (`matric-inference`) automatically benefits from these Ollama optimizations without code changes.
...
Check that Matric Memory can use optimized Ollama:
