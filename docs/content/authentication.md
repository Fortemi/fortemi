...
Matric Memory supports two authentication mechanisms: **API Keys** (simple, token-based) and **OAuth2** (full authorization flow with PKCE). Choose the method that best fits your use case.
...
```bash
# Create an API key
curl -X POST https://memory.integrolabs.net/api/v1/api-keys \
...
```bash
curl https://memory.integrolabs.net/api/v1/notes \
...
API_BASE = "https://memory.integrolabs.net"
API_KEY = "mm_key_xYz9Q4rTp2Lm8vBn3HjK6WcE1DfG5sA7"
...
**JavaScript Example:**
```javascript
const API_BASE = "https://memory.integrolabs.net";
...
Matric Memory implements **OAuth 2.0** with:
...
```json
{
  "issuer": "https://memory.integrolabs.net",
  "authorization_endpoint": "https://memory.integrolabs.net/oauth/authorize",
  "token_endpoint": "https://memory.integrolabs.net/oauth/token",
  "registration_endpoint": "https://memory.integrolabs.net/oauth/register",
  "introspection_endpoint": "https://memory.integrolabs.net/oauth/introspect",
  "revocation_endpoint": "https://memory.integrolabs.net/oauth/revoke",
...
```json
{
  ...
  "registration_client_uri": "https://memory.integrolabs.net/oauth/register/mm_AbCdEfGh12345678901234"
}
```
...
response = requests.post(
    "https://memory.integrolabs.net/oauth/token",
    headers={
...
**Request:**
```bash
curl -X POST https://memory.integrolabs.net/oauth/introspect \
  -H "Authorization: Basic $(echo -n 'client_id:client_secret' | base64)" \
  -H "Content-Type: application/x-www-form-urlencoded" \
...
**Request:**
```bash
curl -X POST https://memory.integrolabs.net/oauth/revoke \
  -H "Authorization: Basic $(echo -n 'client_id:client_secret' | base64)" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "token=mm_at_xYz9Q4rTp2Lm8vBn3HjK6WcE1DfG5sA7UiO0pN9qR" \
...
Matric Memory uses OAuth2 scopes to control access levels.
...
class MatricMemoryClient:
    def __init__(self, client_id, client_secret, redirect_uri):
        ...
        self.base_url = "https://memory.integrolabs.net"
