...
Run `scripts/init-bundle-env.sh` once before the first bundle startup, then run
`scripts/validate-bundle-exposure.sh` before startup and reset. The bootstrap
is idempotent, replaces empty or known reusable password defaults, enforces
mode `0600`, and never prints the secret. The exposure
...
authentication, public HTTPS issuer/resource/origin values, a generated or
