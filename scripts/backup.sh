...
#   -c, --config FILE    Configuration file (default: /etc/matric-memory/backup.conf)
...
#   BACKUP_DEST          Local backup directory (default: /var/backups/matric-memory)
...
# Script metadata
readonly SCRIPT_NAME="matric-backup"
readonly SCRIPT_VERSION="1.0.0"

# Default configuration
CONFIG_FILE="${MATRIC_BACKUP_CONFIG:-/etc/matric-memory/backup.conf}"
...
show_help() {
    cat <<EOF
$SCRIPT_NAME v$SCRIPT_VERSION - Matric Memory Database Backup
...
Environment Variables:
  ...
  LOG_FILE               Log file path (default: /var/log/matric-memory/backup.log)
...
Configuration File:
  The script looks for /etc/matric-memory/backup.conf by default.
  This file can contain any of the environment variables above.
  Example: BACKUP_DEST=/mnt/backups/matric
...
cleanup_old_backups() {
    ...
    if [[ "$DRY_RUN" == "true" ]]; then
        ...
        find "$BACKUP_DEST" -name "matric_backup_*.sql*" -type f -mtime "+$BACKUP_RETAIN" 2>/dev/null | while read -r file; do
    ...
    if [[ -n "$BACKUP_REMOTE_RSYNC" ]]; then
        ...
        if ssh -o ConnectTimeout=5 "$remote_host" "find $remote_path -name 'matric_backup_*.sql*' -mtime +$BACKUP_RETAIN -delete" 2>/dev/null; then
...
cleanup_temp() {
    if [[ -d "$BACKUP_TEMP_DIR" ]]; then
        rm -f "${BACKUP_TEMP_DIR}"/matric_backup_*.sql* 2>/dev/null || true
    fi
}
...
main() {
    ...
    local base_filename="matric_backup_${timestamp}.sql"

    log "Starting Matric Memory backup..."
