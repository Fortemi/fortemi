...
incomplete-profile rejection, clean import, and repeated replace convergence.
...
accepts only this complete inventory. This fixture is not an integrated golden
archive or conformance claim.

### full-v1-integrated-candidate.shard

A reproducible signed route fixture generated by
...
sidecar deduplication, and archive read/write/read equality. A database-backed
...
including the persisted 768-dimensional vector boundary. This directory is not
a standalone conformance archive.
...
- `schema-2-runtime.implementation-receipt.json` binds both archives, the
  authority and package identities, the Fortemi production-signing commit and
  immutable sidecar release, the released HotM live clean-recovery receipt,
  and the bounded non-advertisement decision.
- `schema-2-full-v1.interop-receipt.json` pairs that runtime receipt with the
  delivered React advertisement receipt and its five exact
