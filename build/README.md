# matric-memory Build Infrastructure

This directory contains the Docker builder pattern infrastructure for matric-memory CI/CD.
...
docker run --rm \
    ...
    -w /build \
    matric-builder \
    docker build -t matric-memory:local .
```
...
3. Push to registry:
   ...
   docker tag matric-builder:test \
       git.integrolabs.net/roctinam/matric-memory/builder:latest
   docker push git.integrolabs.net/roctinam/matric-memory/builder:latest
   ```
